...
class SMTPProxy(TCPProxy):
    ...
    async def strip(self, client_reader, client_writer):
        ...
        while (not server_reader.at_eof() and
            ...
            if ehlo_response.group('message') and \
               ehlo_response.group('message').upper() == 'STARTTLS':
                server_writer.writer('STARTTLS\r\n'.encode('ascii'))
                await server_writer.drain()
                ...
                if tls_started_re and \
                   tls_started_re.group('status') == '220':
        ...
        while not server_reader.at_eof():
            ...
            client_data_re = regex.SMTP_COMMAND.fullmatch(client_data.deocde('ascii'))
            ...
            if client_data_re:
                if client_data_re.group('cmd').upper() == 'EHLO':
                    # handle EHLO
                    server_response = await server_reader.readline()
                    server_response_re = regex.SMTP_RESPONSE.fullmatch(server_response.decode('ascii'))
                    logging.debug('Received server data: {}'.format(server_data))
                    client_writer.write(server_data)
                    logging.debug('Writing server data to client...')
                    await client_writer.drain()
                    ...
                    while (not server_reader.at_eof() and
                           server_response_re and
                           server_response_re.group('line_cont')):
                        server_response = await server_reader.readline()
                        server_response_re = regex.SMTP_RESPONSE.fullmatch(server_response.decode('ascii'))
                        logging.debug('Received server data: {}'.format(server_data))
                        client_writer.write(server_data)
